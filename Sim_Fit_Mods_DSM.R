...
fun_DSM_est = function(mod, df, mod_name, g_pop, n_boot){
  ...
  time_ext = filter(new_df, Time > max_fu)
    ...
    df_tmp = pmap_dfc(tmp_full, function(Level, Trend) Level + Trend * (log1p(time_ext$Time) - log1p(max_fu)))
    df_out = tibble(time = time_ext$Time, est = exp(apply(df_tmp, 1, mean)),
                    lcl = exp(apply(df_tmp, 1, quantile, 0.025)),
                    ucl = exp(apply(df_tmp, 1, quantile, 1-0.025)), Model = mod_name)
...
temp_mod <- stan(
  file = here("DSM", "Trend_GenPop.stan"), # Stan program
  data = my_data,         # named list of data
  chains = 1
)
set.seed(675)
df_DRSM = mutate(df_sims, mod = map(df_Agg, function(x) mod_DRSM(x, "Trend_GenPop.stan")),
                 mod_haz = map2(df_Agg, mod, function(x, y) fun_DSM_est(y, tail(x, -1), "Trend", Gen_pop, 1)), Model = "DRSM")

write_rds(df_DRSM, here("df_DRSM.rds"))
